...
using Bucket.Rpc.ProxyGenerator.Utilitys;
        ...
        private static SyntaxList<UsingDirectiveSyntax> GetUsings()
        {
            return List(
                new[]
                {
                    ...
                    UsingDirective(GetQualifiedNameSyntax(typeof(EndPoint).Namespace)),
        ...
        private MemberDeclarationSyntax GenerateMethodDeclaration(MethodInfo method)
        {
            ...
            if (method.ReturnType != typeof(Task))
            {
                expressionSyntax = GenericName(
                    Identifier("InvokeAsync")).WithTypeArgumentList(((GenericNameSyntax)returnDeclaration).TypeArgumentList);
            }
            else
            {
